...
class Pose_estimation_mode:
	def __init__(self):
		#self.base_dir =  os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
		self.base_dir = 'D:\mine\diploma\Models'

		#print(os.path.abspath(__file__))
		# base_model = load_model("../model/pose_net.cntkmodel")
		base_model = C.load_model(os.path.join(self.base_dir, "pose_net.model"))
		data = C.input_variable(shape=(3, C.FreeDimension, C.FreeDimension), name="data") #FreeDimension means the dimension can change for different minibatch

		def clone_model(base_model, from_node_names, to_node_names, clone_method):
			from_nodes = [C.logging.find_by_name(base_model, node_name) for node_name in from_node_names]
			...
			to_nodes = [C.logging.find_by_name(base_model, node_name) for node_name in to_node_names]
			if None in to_nodes:
				print("Error: could not find all specified 'to_nodes' in clone. Looking for {}, found {}"
						.format(to_node_names, to_nodes))
			input_placeholders = dict(zip(from_nodes, [C.placeholder() for x in from_nodes]))
			cloned_net = C.combine(to_nodes).clone(clone_method, input_placeholders)
			return cloned_net

		predictor = clone_model(base_model, ['data'], ["Mconv7_stage6_L1", "Mconv7_stage6_L2"], C.CloneMethod.freeze)
	...
	def process(self, oriImg):
		#canvas = oriImg.copy()
		#canvas = np.empty(oriImg.shape)
		canvas = np.full(oriImg.shape, 255, dtype=np.uint8)
...
if __name__ == '__main__':
	...
	pose = Pose_estimation_mode()
	# test_image = '../sample/ski.jpg'
	test_image = os.path.join(r'E:\Project\mine\diploma\Dataset\Softmax\Pictures', 'sith.jpg')
